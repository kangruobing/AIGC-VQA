...
def load_model(model_path, device):
    ...
    model = Temporalmodule(split_id=0)
...
def predict_single_model(model, dataloader, device):
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc="预测中..."):
            ...
            predicted_score = model.forward(video)
...
def main():
    ...
    parser.add_argument('--output_dir', type=str, default='/root/autodl-tmp/VQualA/temporal_module/results')
    parser.add_argument('--output_file', type=str, default='temporal_predictions.csv')
    ...
    test_dataset = DatasetVideo('test', 0, video_clip_length=24)
